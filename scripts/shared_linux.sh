...
create_test_file_entries_with_extended_attributes()
{
    ...
    touch "${FILE_NAME}"
    setfattr -n "user.myxattr1" -v "My 1st extended attribute" "${FILE_NAME}"
    ...
    FILE_NAME="${MOUNT_POINT}/testdir1/uninitialized1"
    fallocate -x -l 4096 "${FILE_NAME}"
    echo "File with an uninitialized extent" >> "${FILE_NAME}"
