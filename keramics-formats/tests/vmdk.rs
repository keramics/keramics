...
fn read_media_from_image(image: &mut VmdkImage) -> Result<(u64, String), ErrorTrace> {
        ...
        let read_count = match image.read(&mut data) {
            ...
            Err(mut error) => {
                keramics_core::error_trace_add_frame!(
                    error,
                    format!(
                        "Unable to read from VMDK image at offset {} (0x{:08x})",
...
fn read_media() -> Result<(), ErrorTrace> {
    ...
    let mut image: VmdkImage = open_image(&path_buf, "ext2.vmdk")?;
