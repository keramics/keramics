...
use keramics_types::{bytes_to_u16_be, bytes_to_u64_be};
...
#[layout_map(
    structure(
        ...
        field(name = "previous_btree_block_number", data_type = "u64"),
        field(name = "next_btree_block_number", data_type = "u64"),
        group(
            size_condition = ">= 72",
            ...
            field(name = "owner_allocation_group", data_type = "u64"),
...
pub struct XfsBtreeNodeHeader64bit {
    ...
    pub previous_btree_block_number: u64,

    /// Next B-tree block number.
    pub next_btree_block_number: u64,
}

impl XfsBtreeNodeHeader64bit {
    ...
    pub fn read_data(&mut self, data: &[u8]) -> Result<(), ErrorTrace> {
        ...
        if data_size != 24 && data_size != 72 {
            return Err(keramics_core::error_trace_new!("Unsupported data size"));
        }
        self.signature = data[0..4].to_vec();
        self.level = bytes_to_u16_be!(data, 4);
        self.number_of_records = bytes_to_u16_be!(data, 6);
        self.previous_btree_block_number = bytes_to_u64_be!(data, 8);
        self.next_btree_block_number = bytes_to_u64_be!(data, 16);
...
mod tests {
    ...
    fn test_read_data() -> Result<(), ErrorTrace> {
        ...
        let mut test_struct = XfsBtreeNodeHeader64bit::new();
        ...
        assert_eq!(test_struct.previous_btree_block_number, 0xffffffffffffffff);
        assert_eq!(test_struct.next_btree_block_number, 0xffffffffffffffff);
