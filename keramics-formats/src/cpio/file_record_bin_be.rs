...
/// Copy in and out (CPIO) bit-endian binary file record
pub struct CpioBinaryFileRecordBigEndian {}

impl CpioBinaryFileRecordBigEndian {
    /// Reads the record from a buffer.
    pub fn read_data(file_record: &mut CpioFileRecord, data: &[u8]) -> Result<(), ErrorTrace> {
        if data.len() < 26 {
            return Err(keramics_core::error_trace_new!("Unsupported data size"));
        }
        if &data[0..2] != &[0x71, 0xc7] {
            return Err(keramics_core::error_trace_new!("Unsupported signature"));
        }
        file_record.inode_number = bytes_to_u16_be!(data, 4) as u32;
        file_record.file_mode = bytes_to_u16_be!(data, 6) as u32;
        file_record.owner_identifier = bytes_to_u16_be!(data, 8) as u32;
        file_record.group_identifier = bytes_to_u16_be!(data, 10) as u32;
        file_record.number_of_links = bytes_to_u16_be!(data, 12) as u32;
        file_record.device_identifier = bytes_to_u16_be!(data, 14) as u32;

        let value_16bit_upper: u32 = bytes_to_u16_be!(data, 16) as u32;
        let value_16bit_lower: u32 = bytes_to_u16_be!(data, 18) as u32;
        ...
        file_record.path_size = bytes_to_u16_be!(data, 20) as u32;

        let value_16bit_upper: u32 = bytes_to_u16_be!(data, 22) as u32;
        let value_16bit_lower: u32 = bytes_to_u16_be!(data, 24) as u32;
...
mod tests {
    ...
    fn get_test_data() -> Vec<u8> {
        vec![
            0x71, 0xc7, 0x07, 0x00, 0x00, 0x12, 0x81, 0xa4, 0x03, 0xe8, 0x03, 0xe8, 0x00, 0x01,
            0x00, 0x00, 0x67, 0x78, 0xea, 0x09, 0x00, 0x1d, 0x00, 0x00, 0x00, 0x00, 0x6d, 0x2f,
            0x74, 0x6e, 0x6b, 0x2f, 0x72, 0x65, 0x6d, 0x61, 0x63, 0x69, 0x2f, 0x73, 0x66, 0x6e,
            0x5f, 0x63, 0xc3, 0x74, 0x73, 0xa9, 0x66, 0x74, 0x6c, 0x69, 0xa8, 0xc3, 0x00, 0x00,
        ]
    }
    ...
    fn test_read_data() -> Result<(), ErrorTrace> {
        ...
        CpioBinaryFileRecordBigEndian::read_data(&mut test_struct, &test_data)?;
    ...
    fn test_read_data_with_unsupported_data_size() {
        ...
        let result: Result<(), ErrorTrace> =
            CpioBinaryFileRecordBigEndian::read_data(&mut test_struct, &test_data[0..25]);
