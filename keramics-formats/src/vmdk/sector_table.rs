...
/// VMware Virtual Disk (VMDK) sector table entry.
pub struct VmdkSectorTableEntry {
    // Sector number.
    pub sector_number: u32,
}

impl VmdkSectorTableEntry {
    /// Creates a sector table entry.
    pub fn new() -> Self {
        Self { sector_number: 0 }
    }

    /// Reads the sector table entry from a buffer.
    pub fn read_data(&mut self, data: &[u8]) -> Result<(), ErrorTrace> {
        if data.len() != 4 {
            return Err(keramics_core::error_trace_new!(
                "Unsupported sector table entry data size"
            ));
        }
...
/// VMware Virtual Disk (VMDK) sector table.
pub struct VmdkSectorTable {
    /// Offset.
    offset: u64,
...
impl VmdkSectorTable {
    ...
    pub fn read_entry(
        ...
        entry_index: u32,
    ) -> Result<VmdkSectorTableEntry, ErrorTrace> {
        ...
        match entry.read_data(&data) {
            Ok(_) => {}
            Err(mut error) => {
                keramics_core::error_trace_add_frame!(error, "Unable to read sector table");
                return Err(error);
            }
        }
        Ok(entry)
...
mod tests {
    ...
    fn test_read_data() -> Result<(), ErrorTrace> {
        ...
        let mut test_struct = VmdkSectorTableEntry::new();
    ...
    fn test_read_entry() -> Result<(), ErrorTrace> {
        ...
        let mut test_struct = VmdkSectorTable::new();
        test_struct.set_range(0, 4);

        let test_entry: VmdkSectorTableEntry = test_struct.read_entry(&data_stream, 0)?;
        assert_eq!(test_entry.sector_number, 22);

        let test_entry: VmdkSectorTableEntry = test_struct.read_entry(&data_stream, 1)?;
