...
/// Expert Witness Compression Format (EWF) table footer.
pub struct EwfTableFooter {
    /// Checksum.
    pub checksum: u32,
}

impl EwfTableFooter {
    /// Creates a new table footer.
    pub fn new() -> Self {
        Self { checksum: 0 }
    }
    ...
    pub fn read_data(&mut self, data: &[u8]) -> io::Result<()> {
        if data.len() < 4 {
            return Err(io::Error::new(
                ...
                format!("Unsupported EWF table footer data size"),
            ));
        }
...
mod tests {
    ...
    fn test_read_data() -> io::Result<()> {
        ...
        let mut test_struct = EwfTableFooter::new();
        test_struct.read_data(&test_data)?;

        assert_eq!(test_struct.checksum, 0x60d56934);
