...
#[layout_map(
    structure(
        ...
        field(name = "extent_offset", data_type = "u64"),
    ),
    methods("debug_read_data")
)]
/// Apple File System (APFS) file system key with extent.
pub struct ApfsFileSystemKeyWithExtent {
    /// Data type.
    pub data_type: u8,
...
impl ApfsFileSystemKeyWithExtent {
    /// Creates a new key.
    pub fn new() -> Self {
        Self {
            data_type: 0,
    ...
    pub fn read_data(&mut self, data: &[u8]) -> Result<(), ErrorTrace> {
        ...
        self.extent_offset = bytes_to_u64_le!(data, 8);
...
mod tests {
    ...
    fn get_test_data() -> Vec<u8> {
        return vec![
            0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x90, 0x09, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00,
        ];
    }
    ...
    fn test_read_data() -> Result<(), ErrorTrace> {
        ...
        let mut test_struct = ApfsFileSystemKeyWithExtent::new();
        test_struct.read_data(&test_data)?;

        assert_eq!(test_struct.data_type, 9);
        assert_eq!(test_struct.object_identifier, 2);
        assert_eq!(test_struct.extent_offset, 9);
