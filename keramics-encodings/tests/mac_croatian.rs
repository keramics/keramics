...
const MAC_CROATIAN_TEST_VECTOR: [(u32, &'static [u8]); 256] = [
    (0x0000, &[0x00]),
    (0x0001, &[0x01]),
    (0x0002, &[0x02]),
    (0x0003, &[0x03]),
...
fn decode() -> Result<(), ErrorTrace> {
    for (expected_code_point, test_byte_string) in MAC_CROATIAN_TEST_VECTOR.iter() {
...
fn encode() -> Result<(), ErrorTrace> {
    for (test_code_point, expected_byte_string) in MAC_CROATIAN_TEST_VECTOR.iter() {
