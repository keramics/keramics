...
const MAC_ARABIC_TEST_VECTOR: [(&'static [u32], &'static [u8], bool); 256] = [
    (&[0x0000], &[0x00], false),
    (&[0x0001], &[0x01], false),
    (&[0x0002], &[0x02], false),
    (&[0x0003], &[0x03], false),
...
fn decode() -> Result<(), ErrorTrace> {
    for (expected_code_points, test_byte_string, _) in MAC_ARABIC_TEST_VECTOR.iter() {
...
fn encode() -> Result<(), ErrorTrace> {
    for (test_code_points, expected_byte_string, is_duplicate) in MAC_ARABIC_TEST_VECTOR.iter() {
        let mut encoder: EncoderMacArabic = EncoderMacArabic::new(test_code_points);
