...
const KOI8U_TEST_VECTOR: [(&'static [u32], &'static [u8]); 256] = [
    (&[0x0000], &[0x00]),
    (&[0x0001], &[0x01]),
    (&[0x0002], &[0x02]),
    (&[0x0003], &[0x03]),
...
fn decode() -> Result<(), ErrorTrace> {
    for (expected_code_points, test_byte_string) in KOI8U_TEST_VECTOR.iter() {
...
fn encode() -> Result<(), ErrorTrace> {
    for (test_code_points, expected_byte_string) in KOI8U_TEST_VECTOR.iter() {
        let mut encoder: EncoderKoi8U = EncoderKoi8U::new(test_code_points);
