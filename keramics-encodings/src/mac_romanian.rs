...
//! MacRomanian encoding.
//!
//! Provides support for encoding and decoding MacRomanian.

use keramics_core::ErrorTrace;

/// MacRomanian decoder.
pub struct DecoderMacRomanian<'a> {
...
impl<'a> DecoderMacRomanian<'a> {
    const BASE_0X80: [u16; 128] = [
        ...
        0x00a8, 0x2260, 0x0102, 0x0218, 0x221e, 0x00b1, 0x2264, 0x2265, 0x00a5, 0x00b5, 0x2202,
        0x2211, 0x220f, 0x03c0, 0x222b, 0x00aa, 0x00ba, 0x03a9, 0x0103, 0x0219, 0x00bf, 0x00a1,
        ...
        0x00ff, 0x0178, 0x2044, 0x20ac, 0x2039, 0x203a, 0x021a, 0x021b, 0x2021, 0x00b7, 0x201a,
...
impl<'a> Iterator for DecoderMacRomanian<'a> {
...
/// MacRomanian encoder.
pub struct EncoderMacRomanian<'a> {
    /// Code points.
    code_points: &'a [u32],
...
impl<'a> EncoderMacRomanian<'a> {
    const BASE_0X00A0: [Option<&'static [u8]>; 104] = [
        Some(&[0xca]),
        Some(&[0xc1]),
        Some(&[0xa2]),
        Some(&[0xa3]),
...
impl<'a> Iterator for EncoderMacRomanian<'a> {
        ...
        match self.code_points.get(self.code_point_index) {
            Some(code_point) => {
                ...
                match *code_point {
                    ...
                    0x00a0..0x0108 => match Self::BASE_0X00A0[(*code_point - 0x00a0) as usize] {
                        Some(bytes) => Some(Ok(bytes.to_vec())),
                        None => Some(Err(keramics_core::error_trace_new!(format!(
                            "Unable to encode code point: U+{:04x} as MacRomanian",
                            *code_point
                        )))),
                    },
                    0x2010..0x2048 => match Self::BASE_0X2010[(*code_point - 0x2010) as usize] {
                        Some(bytes) => Some(Ok(bytes.to_vec())),
                        None => Some(Err(keramics_core::error_trace_new!(format!(
                            "Unable to encode code point: U+{:04x} as MacRomanian",
                            *code_point
                    ...
                    0x0218 => Some(Ok(vec![0xaf])),
                    0x0219 => Some(Ok(vec![0xbf])),
                    0x021a => Some(Ok(vec![0xde])),
                    0x021b => Some(Ok(vec![0xdf])),
                    0x02c6 => Some(Ok(vec![0xf6])),
                    0x02c7 => Some(Ok(vec![0xff])),
                    0x02d8 => Some(Ok(vec![0xf9])),
                    0x02d9 => Some(Ok(vec![0xfa])),
                    0x02da => Some(Ok(vec![0xfb])),
                    0x02db => Some(Ok(vec![0xfe])),
                    0x02dc => Some(Ok(vec![0xf7])),
                    0x02dd => Some(Ok(vec![0xfd])),
...
mod tests {
    ...
    fn test_decode() -> Result<(), ErrorTrace> {
        ...
        let mut decoder: DecoderMacRomanian = DecoderMacRomanian::new(&byte_string);
    ...
    fn test_encode() -> Result<(), ErrorTrace> {
        ...
        let mut encoder: EncoderMacRomanian = EncoderMacRomanian::new(&code_points);
