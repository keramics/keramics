...
impl<'a> DecoderMacSymbol<'a> {
    const BASE_0X20: [Option<&'static [u16]>; 224] = [
        Some(&[0x0020]),
        Some(&[0x0021]),
        Some(&[0x2200]),
        Some(&[0x0023]),
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self.bytes.get(self.byte_index) {
            Some(byte_value) => {
                ...
                let code_points: &[u16] = if *byte_value < 0x20 {
                    &[*byte_value as u16]
                } else {
        ...
        match self.code_points.get(self.code_point_index) {
            Some(code_point) => {
                ...
                match *code_point {
                    ...
                    0x00ae => match self.code_points.get(self.code_point_index) {
                        Some(0xf87f) => {
                            self.code_point_index += 1;
                    ...
                    0x2122 => match self.code_points.get(self.code_point_index) {
                        Some(0xf87f) => {
                            self.code_point_index += 1;
