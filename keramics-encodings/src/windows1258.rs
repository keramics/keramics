...
//! Windows 1258 (Vietnamese) encoding.
//!
//! Provides support for encoding and decoding Windows 1258.

use keramics_core::ErrorTrace;

/// Windows 1258 decoder.
pub struct DecoderWindows1258<'a> {
...
impl<'a> Iterator for DecoderWindows1258<'a> {
                ...
                if *byte_value < 0x80 {
                    ...
                    match Self::BASE_0X80[(*byte_value - 0x80) as usize] {
                        ...
                        None => Some(Err(keramics_core::error_trace_new!(format!(
                            "Unable to decode Windows 1258: 0x{:02x} as Unicode",
                            *byte_value
                        )))),
                    }
                }
...
/// Windows 1258 encoder.
pub struct EncoderWindows1258<'a> {
    /// Code points.
    code_points: &'a [u32],
...
impl<'a> EncoderWindows1258<'a> {
    const BASE_0X00C0: [Option<&'static [u8]>; 72] = [
        Some(&[0xc0]),
        Some(&[0xc1]),
        Some(&[0xc2]),
        None,
...
impl<'a> Iterator for EncoderWindows1258<'a> {
        ...
        match self.code_points.get(self.code_point_index) {
            Some(code_point) => {
                ...
                match *code_point {
                    ...
                    0x01a0 => Some(Ok(vec![0xd5])),
                    0x01a1 => Some(Ok(vec![0xf5])),
                    0x01af => Some(Ok(vec![0xdd])),
                    0x01b0 => Some(Ok(vec![0xfd])),
                    0x02c6 => Some(Ok(vec![0x88])),
                    0x02dc => Some(Ok(vec![0x98])),
                    0x0300 => Some(Ok(vec![0xcc])),
                    0x0301 => Some(Ok(vec![0xec])),
                    0x0303 => Some(Ok(vec![0xde])),
                    0x0309 => Some(Ok(vec![0xd2])),
                    0x0323 => Some(Ok(vec![0xf2])),
...
mod tests {
    ...
    fn test_decode() -> Result<(), ErrorTrace> {
        ...
        let mut decoder: DecoderWindows1258 = DecoderWindows1258::new(&byte_string);
    ...
    fn test_encode() -> Result<(), ErrorTrace> {
        ...
        let mut encoder: EncoderWindows1258 = EncoderWindows1258::new(&code_points);
