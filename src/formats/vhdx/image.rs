...
impl VfsFileSystem for VhdxImage {
    ...
    fn open_file_entry(
        ...
        let layer: Option<VhdxLayer> = match self.get_layer_by_path(&path.location) {
            Ok(layer) => layer,
...
mod tests {
    ...
    fn test_file_entry_exists() -> io::Result<()> {
        ...
        let vfs_path: VfsPathReference = VfsPath::new(VfsPathType::Vhdx, "/vhdx1", None);
        assert_eq!(image.file_entry_exists(&vfs_path)?, true);

        let vfs_path: VfsPathReference = VfsPath::new(VfsPathType::Vhdx, "./bogus2", None);
    ...
    fn test_open_file_entry_of_file() -> io::Result<()> {
        ...
        let test_vfs_path: VfsPathReference =
            VfsPath::new(VfsPathType::Vhdx, "/vhdx1", Some(&os_vfs_path));
        let vfs_file_entry: VfsFileEntryReference = image.open_file_entry(&test_vfs_path)?.unwrap();
    ...
    fn test_open_file_entry_non_existing() -> io::Result<()> {
        ...
        let test_vfs_path: VfsPathReference =
            VfsPath::new(VfsPathType::Vhdx, "/bogus1", Some(&os_vfs_path));
