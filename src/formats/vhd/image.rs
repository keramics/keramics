...
impl VhdImage {
    ...
    pub fn get_layer_by_index(&self, layer_index: usize) -> io::Result<VhdLayer> {
        match self.files.get(layer_index) {
            Some(file) => {
                let mut layer: VhdLayer = VhdLayer::new();
                layer.open(&file)?;
    ...
    fn get_layer_by_path(&self, location: &str) -> io::Result<Option<VhdLayer>> {
        if location == "/" {
            return Ok(None);
        }
...
impl VfsFileSystem for VhdImage {
    ...
    fn open_file_entry(&self, path: &VfsPath) -> io::Result<VfsFileEntryReference> {
        ...
        let layer: Option<VhdLayer> = self.get_layer_by_path(&path.location)?;

        let mut file_entry: WrapperVfsFileEntry = WrapperVfsFileEntry::new::<VhdLayer>(layer);
...
mod tests {
    ...
    fn test_get_directory_name() -> io::Result<()> {
        let image = VhdImage::new();

        let directory_name: &str = image.get_directory_name("/vhd1");
