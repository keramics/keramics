                            ...
                            match self.volume_system.get_partition_by_index(partition_index) {
                                ...
                                Err(mut error) => {
                                    keramics_core::error_trace_add_frame!(
                                        error,
                                        format!(
                                            "Unable to retrieve GPT partition: {}",
                                            partition_index
                                        )
                                    );
                                    return Err(error);
...
mod tests {
    ...
    fn test_file_entry_exists() -> Result<(), ErrorTrace> {
        ...
        let vfs_path: VfsPath = VfsPath::from_string(&VfsType::Mbr, "/mbr1");
        let result: bool = mbr_file_system.file_entry_exists(&vfs_path)?;
        assert_eq!(result, true);

        let vfs_path: VfsPath = VfsPath::from_string(&VfsType::Mbr, "/bogus1");
        let result: bool = mbr_file_system.file_entry_exists(&vfs_path)?;
        assert_eq!(result, false);

        let vfs_path: VfsPath = VfsPath::from_string(&VfsType::Mbr, "/mbr1/bogus1");
        let result: bool = mbr_file_system.file_entry_exists(&vfs_path)?;
        assert_eq!(result, false);
        ...
        let result: Result<bool, ErrorTrace> = mbr_file_system.file_entry_exists(&vfs_path);
