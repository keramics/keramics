...
use super::file_entry::LuksFileEntry;

/// Linux Unified Key Setup (LUKS) Disk Encryption encrypted volume file system.
...
impl LuksFileSystem {
    pub const PATH_PREFIX: &'static str = "/luks";

    /// Creates a new file system.
    pub fn new() -> Self {
        Self {
            encrypted_volume: Arc::new(LuksEncryptedVolume::new()),
        }
    }
    ...
    pub fn get_file_entry_by_path(&self, path: &Path) -> Result<Option<LuksFileEntry>, ErrorTrace> {
        if path.is_relative() {
            return Ok(None);
        }
        match path.get_component_by_index(1) {
            Some(path_component) => {
                ...
                if path_component != "luks1" {
                    return Ok(None);
                }
                Ok(Some(LuksFileEntry::UnlockedVolume {
    ...
    pub fn get_root_file_entry(&self) -> LuksFileEntry {
        LuksFileEntry::Root {
            encrypted_volume: self.encrypted_volume.clone(),
        }
    }
    ...
    pub fn open(
        ...
        match Arc::get_mut(&mut self.encrypted_volume) {
            ...
            None => {
                return Err(keramics_core::error_trace_new!(
                    "Unable to obtain mutable reference to LUKS encrypted volume"
                ));
            }
        }
        Ok(())
    ...
    pub(crate) fn open_encrypted_volume(
        encrypted_volume: &mut LuksEncryptedVolume,
        ...
        if encrypted_volume.is_locked() {
            ...
            match encrypted_volume.unlock(&luks_credentials) {
                ...
                Err(mut error) => {
                    keramics_core::error_trace_add_frame!(
                        error,
                        "Failed to unlock LUKS encrypted volume"
                    );
                    return Err(error);
                }
            }
...
mod tests {
    ...
    fn get_file_system() -> Result<LuksFileSystem, ErrorTrace> {
        let mut luks_file_system: LuksFileSystem = LuksFileSystem::new();
        ...
        luks_file_system.open(Some(&os_file_system), &os_vfs_location)?;
    ...
    fn test_file_entry_exists() -> Result<(), ErrorTrace> {
        let luks_file_system: LuksFileSystem = get_file_system()?;

        let path: Path = Path::from("/");
        let result: bool = luks_file_system.file_entry_exists(&path);
        ...
        let path: Path = Path::from("/luks99");
        let result: bool = luks_file_system.file_entry_exists(&path);
        ...
        let path: Path = Path::from("/luks1/bogus1");
        let result: bool = luks_file_system.file_entry_exists(&path);
        assert_eq!(result, false);
    ...
    fn test_get_file_entry_by_path() -> Result<(), ErrorTrace> {
        ...
        let path: Path = Path::from("/");
        let result: Option<LuksFileEntry> = luks_file_system.get_file_entry_by_path(&path)?;
        assert!(result.is_some());

        let luks_file_entry: LuksFileEntry = result.unwrap();

        let name: PathComponent = luks_file_entry.get_name();
        assert_eq!(name, PathComponent::Root);

        let file_type: VfsFileType = luks_file_entry.get_file_type();
    ...
    fn test_get_root_file_entry() -> Result<(), ErrorTrace> {
        ...
        assert!(matches!(luks_file_entry, LuksFileEntry::Root { .. }));
