...
/// Mac OS sparse image (.sparsebundle) storage media image file entry.
pub enum SparseBundleFileEntry {
    /// Layer file entry.
    Layer {
        /// File.
        image: Arc<RwLock<SparseBundleImage>>,
...
impl SparseBundleFileEntry {
    /// Retrieves the default data stream.
    pub fn get_data_stream(&self) -> Result<Option<DataStreamReference>, ErrorTrace> {
        match self {
            SparseBundleFileEntry::Layer { image, .. } => Ok(Some(image.clone())),
    ...
    pub fn get_file_type(&self) -> VfsFileType {
        match self {
            SparseBundleFileEntry::Layer { .. } => VfsFileType::File,
            SparseBundleFileEntry::Root { .. } => VfsFileType::Directory,
        }
    }
    ...
    pub fn get_name(&self) -> PathComponent {
        match self {
            SparseBundleFileEntry::Layer { .. } => PathComponent::from("sparsebundle1"),
            SparseBundleFileEntry::Root { .. } => PathComponent::Root,
        }
    }
    ...
    pub fn get_size(&self) -> u64 {
        match self {
            SparseBundleFileEntry::Layer { size, .. } => *size,
    ...
    pub fn get_number_of_sub_file_entries(&self) -> usize {
        match self {
            SparseBundleFileEntry::Layer { .. } => 0,
            SparseBundleFileEntry::Root { .. } => 1,
        }
    }
    ...
    ) -> Result<SparseBundleFileEntry, ErrorTrace> {
        match self {
            SparseBundleFileEntry::Layer { .. } => {
                Err(keramics_core::error_trace_new!("No sub file entries"))
            }
            SparseBundleFileEntry::Root { image } => {
                ...
                Ok(SparseBundleFileEntry::Layer {
                    image: image.clone(),
                    size: media_size,
                })
            }
    ...
    pub fn is_root_file_entry(&self) -> bool {
        match self {
            SparseBundleFileEntry::Layer { .. } => false,
            SparseBundleFileEntry::Root { .. } => true,
        }
    }
}
...
mod tests {
    ...
    fn test_get_sub_file_entry_by_index() -> Result<(), ErrorTrace> {
        ...
        let result: Result<SparseBundleFileEntry, ErrorTrace> =
