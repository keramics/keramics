...
struct VhdxFileInfo<'a> {
    /// File.
    file: &'a VhdxFile,
}

impl<'a> VhdxFileInfo<'a> {
    const DISK_TYPES: &'static [(VhdxDiskType, &'static str); 3] = &[
    ...
    fn new(file: &'a VhdxFile) -> Self {
        Self { file }
    }

    /// Retrieves the disk type as a string.
    pub fn get_disk_type_string(&self, disk_type: &VhdxDiskType) -> &str {
        Self::DISK_TYPES
...
impl<'a> fmt::Display for VhdxFileInfo<'a> {
...
/// Information about Virtual Hard Disk (VHDX) format.
pub struct VhdxInfo {}
...
impl VhdxInfo {
    ...
    pub fn print_file(data_stream: &DataStreamReference) -> Result<(), ErrorTrace> {
        ...
        let file_information: VhdxFileInfo = VhdxFileInfo::new(&vhdx_file);
