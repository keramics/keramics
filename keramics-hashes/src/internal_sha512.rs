...
/// Context for calculating a SHA-384 and SHA-512 hash.
pub struct InternalSha512Context {
    /// Hash values.
    hash_values: [u64; 8],
...
impl InternalSha512Context {
    /// Creates a new context.
    pub fn new(prime_square_roots: [u64; 8]) -> Self {
        Self {
            hash_values: prime_square_roots,
    ...
    pub fn finalize(&mut self, prime_square_roots: &[u64]) -> Vec<u8> {
        ...
        self.hash_values.copy_from_slice(prime_square_roots);
    ...
    fn transform_block(
        ...
        for value_64bit in &mut values_64bit[0..16] {
            *value_64bit = bytes_to_u64_be!(data, data_offset);
        ...
        for (value_index, value_64bit) in values_64bit.iter().enumerate() {
    ...
    pub fn update(&mut self, data: &[u8]) {
        ...
        if self.block_offset > 0 {
            let remaining_block_size: usize = SHA512_BLOCK_SIZE - self.block_offset;
            ...
            self.block[self.block_offset..block_end_offset]
                .copy_from_slice(&data[data_offset..data_end_offset]);

            self.block_offset = block_end_offset;
        ...
        let remaining_bytes: usize = data_size - data_offset;
        let block_end_offset: usize = self.block_offset + remaining_bytes;
        let data_end_offset: usize = data_offset + remaining_bytes;
