...
const SHA224_PRIME_CUBE_ROOTS: [u32; 64] = [
...
impl Sha224Context {
    ...
    pub fn new() -> Self {
        Self {
            hash_values: SHA224_PRIME_SQUARE_ROOTS,
            number_of_bytes_hashed: 0,
            block_offset: 0,
            block: [0; SHA224_BLOCK_SIZE * 2],
        }
    }
    ...
    fn transform_block(
        ...
        for value_index in 0..64 {
            ...
            let t1: u32 = block_hashes[7]
                ...
                .wrapping_add(SHA224_PRIME_CUBE_ROOTS[value_index])
...
impl DigestHashContext for Sha224Context {
    ...
    fn finalize(&mut self) -> Vec<u8> {
        ...
        let padding_size: usize = SHA224_BLOCK_SIZE * if self.block_offset >= 56 { 2 } else { 1 };
        ...
        for block_offset in (0..padding_size).step_by(SHA224_BLOCK_SIZE) {
        ...
        let hash: Vec<u8> = self.hash_values[0..7]
            .iter()
            .map(|hash_value| hash_value.to_be_bytes())
            .flatten()
            .collect::<Vec<u8>>();

        self.hash_values = SHA224_PRIME_SQUARE_ROOTS;
    ...
    fn update(&mut self, data: &[u8]) {
        ...
        if self.block_offset > 0 {
            while self.block_offset < SHA224_BLOCK_SIZE {
            ...
            if self.block_offset == SHA224_BLOCK_SIZE {
                let hash_values: [u32; 8] = self.transform_block(&self.hash_values, &self.block, 0);

                self.hash_values.copy_from_slice(&hash_values);
                self.number_of_bytes_hashed += SHA224_BLOCK_SIZE as u64;

                self.block_offset = 0;
            }
        }
        while data_offset + SHA224_BLOCK_SIZE < data_size {
            let hash_values: [u32; 8] = self.transform_block(&self.hash_values, &data, data_offset);

            self.hash_values.copy_from_slice(&hash_values);
            self.number_of_bytes_hashed += SHA224_BLOCK_SIZE as u64;

            data_offset += SHA224_BLOCK_SIZE;
        }
