...
The Volume Master Key (VMK) is 256-bit of size and is stored in multiple Volume Master Key (VMK)
structures. The VMK is stored encrypted with either the recovery password, external key, or the TPM.
...
recovery password is used to determine a recovery password key. Not to be confused with the
"recovery key" key protector.
...
The corresponding recovery password key is calculated using the following approach, written
partially in pseudo C:
...
The salt is stored on disk in the stretch key which is stored in the recovery password key
...
The last SHA-256 value contains the 256-bit key which is recovery pass key that can unlock the
recovery password key protected Volume Master Key (VMK).
...
The external key is used by the "startup key" and "recovery key" key protectors. The external key
is stored in a file named "{%GUID%}.BEK". The GUID in the filename equals the key identifier in the
BitLocker metadata.

There can be multiple external keys for a single BitLocker encrypted volume. Each key is identified
a by a different key identifier.
...
The last SHA-256 value contains the 256-bit key which is user key that can unlock the user key (or
...
AES-CBC (Cipher Block Chaining) with or without Elephant Elephant Diffuser or AES-XTS (XEX-based
tweaked-codebook mode with ciphertext stealing).

To encrypt the key data BitLocker uses AES-CCM (Counter with cipher block chaining message
...
| 21 | 1 | | [Media descriptor](ntfs.md#media_descriptor) |
...
| <td colspan="4">*Common*</td> |
| 90 | 334 | | Bootcode |
| **424** | **16** | | **[BitLocker identifier](#identifiers)**, which contains a GUID |
...
#### Metadata block header version 2 – Windows 7 and later {#metadata_block_header_v2}
...
> Note that when decrypting BitLocker will decrypt from the back to the front. The encrypted
> volume size contains the number of bytes of the volume that are still encrypted (or need to be
> decrypted).
...
| 36 | 2 | | [Encryption method](#encryption_methods) |
| 38 | 2 | | Copy of [encryption method](#encryption_methods) |
...
#### Metadata value types {#value_types}

| Value | Identifier | Description |
| --- | --- | --- |
| 0x0000 | | Erased |
| 0x0001 | | [Key](#key_encrypted_key) |
| 0x0002 | | String, which contains an UCS-2 little-endian string with end-of-string character |
| 0x0003 | | [Stretch Key](#stretch_key) |
| 0x0004 | | Use Key |
| 0x0005 | | [AES-CCM encrypted key](#aes_ccm_encrypted_key) |
| 0x0006 | | [TPM encoded key](#tpm_encoded_key) |
| 0x0007 | | Validation |
| 0x0008 | | [Volume master key](#volume_master_key) |
| 0x0009 | | External key |
| 0x000a | | Update |
| 0x000b | | Error |
| | | |
...
| Offset | Size | Value | Description |
| --- | --- | --- | --- |
| 0 | 2 | | [Encryption method](#encryption_methods) |
| 2 | 2 | | Unknown |
| 4 | ... | | Key data |

### Stretch encrypted key {#stretch_key}
...
| 2 | 2 | | Unknown |
| 4 | 16 | | Salt |
| 20 | ... | | [Metadata entry](#metadata_entry), which contains an [AES-CCM encrypted key](#aes_ccm_encrypted_key) |

### AES-CCM encrypted key {#aes_ccm_encrypted_key}
...
| 6 | 2 | | Unknown |
| 8 | 2 | | [Encryption method](#encryption_methods) |
| 10 | 2 | | Unknown |
| 12 | ... | | Unencrypted key data |

<!-- rumdl-enable MD033 MD056 -->

### TPM encoded key {#tpm_encoded_key}
...
### Volume Master Key (VMK) {#volume_master_key}
...
| 24 | 2 | | Unknown |
| 26 | 2 | | [Protector type](#key_protector_types) |
| 28 | ... | | Properties, which contains an array of [metadata entries](#metadata_entry), where entry type is 0 |
...
#### Key protector types {#key_protector_types}
...
The BEK file header is similar to the metadata header. The BEK file header is 48 bytes in size and
consists of:
...
The format of a BEK metadata entry is similar to the format of [a metadata entry](#metadata_entry).

The metadata in a BEK file consists of an external key, which contains 256-bits of unprotected key
data.
...
| 32 | 4 | | Number of block map area offsets |
