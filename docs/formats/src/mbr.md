...
The Master Boot Record (MBR) partition table is mainly used on the family of Intel x86 based
computers.
...
Traditionally the size of sector is 512 bytes, but modern hard disk drives use 4096 bytes. The
linux fdisk utility supports sector sizes of: 512, 1024, 2048 and 4096.

The location of of the "boot signature" of the MBR does not indicate the sector size. Methods to
derive the sector size from the data:
...
The Master Boot Record (MBR) is a data structure that describes the properties of the storage
medium and its partitions.

The classical MBR can only contain 4 partition table entries. Additional partition entries must be
stored using extended partition records (EPR). The classical MBR has evolved into different
variants like:
...
The extended partition record (EPR) (also referred to as extended boot record (EBR)) starts with a
64 byte (extended) partition record (EPR) like the MBR. This partition table contains information
about the logical partition (volume) and additional extended partition tables.
